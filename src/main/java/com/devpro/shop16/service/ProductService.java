...
import com.devpro.shop16.dto.SearchModel;
...
//				sql += " and p.status=1 ";
	...
	public PagerData<Product> findByCategoryId(int categoryId) {
		String sql = "SELECT * FROM tbl_products p WHERE p.category_id = "+categoryId;
		return executeByNativeSQL(sql,1);
	}
	public PagerData<Product> findByKeyword(String keyword) {
		String sql="SELECT * FROM tbl_products p WHERE LOWER(p.title) like '%"+keyword+"%'" + " or p.detail_description like '%"+keyword+"%'"
				+ " or p.short_description like '%"+keyword+"%'" + " or p.seo like '%"+keyword+"%'";
		return super.executeByNativeSQL(sql,1);
	}
...
//	}
	...
	public List<Product> searchProduct(SearchModel productSearch, int categoryId) {
		//Tao cau lenhj sql
		String sql = "SELECT * FROM tbl_products p WHERE  1 = 1 AND p.category_id = "+categoryId;
